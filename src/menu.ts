export const Menu = {
    ...
    init() {
        ...
        this.bindButton("statsMenuBtn", "statsPanel", () => {
            if ((window as any).syncStatsUI) {
                (window as any).syncStatsUI();
            }
        });
...
if (
    ...
    getNextHint(panelId: string) {
        const hints =
            this.hints[panelId] || this.hints.default;

        const hint =
            hints[this.hintIndex % hints.length];
    ...
    showHint(panel: HTMLElement, panelId: string) {
        if (panelId === "garagePreviewSlot") return;

        let hint =
            panel.querySelector(".menuHint") as HTMLDivElement | null;
