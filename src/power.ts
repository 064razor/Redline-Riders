export type TorqueCurvePoint = [number, number];

export function clamp(value: number, min: number, max: number) {
    return Math.max(min, Math.min(value, max));
}

export function getCurveMultiplier(
    curve: TorqueCurvePoint[] | undefined,
    rpmRatio: number
) {
    const fallbackCurve: TorqueCurvePoint[] = [
        [0, 0.55],
        [0.3, 0.82],
        [0.55, 1.0],
        [0.75, 0.96],
    ...
    for (let i = 1; i < points.length; i++) {
        ...
        if (ratio <= next[0]) {
            const range =
                Math.max(next[0] - previous[0], 0.001);

            const progress =
                (ratio - previous[0]) / range;
...
export function getDisplacementTorqueCurve(
    curve: TorqueCurvePoint[] | undefined,
    level: number
) {
    const points =
...
export function getEstimatedBoostPsi(car: any, rpmRatio: number) {
    const inductionType =
...
export function getBoostTorqueMultiplier(
    car: any,
    boostPsi: number,
    rpmRatio: number
) {
...
export function getEstimatedPowerAtRpm(car: any, rpm: number) {
    const maxRPM =
    ...
    const liveHorsepower =
        liveTorque * Math.max(rpm, 1000) / 5252;
...
export function getShiftWindow(car: any, gear?: number) {
    const maxRPM =
        Math.max(car?.maxRPM || 7000, 1000);

    const minShiftRPM =
        Math.max(1000, maxRPM * 0.24);

    const currentGear =
        Math.max(1, gear || car?.gear || 1);

    const ratios =
        car?.gearRatios || [];

    const oldRatio =
        ratios[currentGear - 1] || ratios[0] || 1;

    const newRatio =
        ratios[currentGear] || oldRatio;
    ...
    if (currentGear < ratios.length) {
        for (let rpm = Math.max(peakRPM, maxRPM * 0.5); rpm <= maxRPM; rpm += 50) {
            const nextGearRPM =
                clamp(rpm * (newRatio / oldRatio), 1000, maxRPM);

            const currentPower =
                getEstimatedPowerAtRpm(car, rpm);

            const nextGearPower =
                getEstimatedPowerAtRpm(car, nextGearRPM);
    ...
    const perfectEnd =
        clamp(bestShiftRPM, minShiftRPM + 400, maxRPM * 0.99);

    const perfectStart =
        clamp(perfectEnd - maxRPM * 0.055, minShiftRPM, perfectEnd - 100);
...
export function getShiftQuality(car: any, rpm: number, gear?: number) {
    const window =
        getShiftWindow(car, gear);

    if (rpm < window.goodStart) return "EARLY";
    if (rpm < window.perfectStart) return "GOOD";
    if (rpm <= window.perfectEnd) return "PERFECT";
