...
function getPaintMaskImage(bodyId, imagePath) {
...
function getDecalImage(decalId, imagePath) {
...
function drawCarBody(ctx, car, x, y, facing) {
    ...
    if (car.underglowColor) {
        const outerGlow = ctx.createRadialGradient(-12, 40, 2, -12, 40, 66);
    ...
    if (shiftJoltDuration > 0 && shiftJoltTimer > 0) {
        const shiftProgress = shiftJoltTimer / shiftJoltDuration;
        const shiftElapsed = 1 - shiftProgress;
        const shiftStrength = car.shiftJoltStrength || 1;
        const rock = Math.sin(shiftElapsed * Math.PI * 2) * 0.007 * shiftStrength;
        ctx.translate(0, 31);
        ctx.rotate(rock);
        ctx.translate(0, -31);
    }
    ...
    // ===== WHEELSPIN SMOKE =====
    if (car.wheelspin) {
        const smokeIntensity = Math.max(0.2, Math.min(car.wheelspinIntensity || 0.45, 1));
        ctx.save();
        ctx.globalAlpha = (0.14 + Math.random() * 0.08) * smokeIntensity;
        ctx.fillStyle = "rgba(225, 225, 225, 0.55)";
        const puffCount = smokeIntensity > 0.62 ? 3 : 2;
        for (let i = 0; i < puffCount; i++) {
            const puffX = poweredWheel.x - 8 - Math.random() * 12;
            const puffY = poweredWheel.y + 3 - Math.random() * 5;
            const puffWidth = poweredWheel.radius * (0.9 + smokeIntensity * 0.75 + Math.random() * 0.55);
            const puffHeight = poweredWheel.radius * (0.42 + smokeIntensity * 0.3 + Math.random() * 0.28);
            ctx.beginPath();
    ...
    if (isImageReady(bodyImage)) {
        ...
        if (car.paintColor && hasPaintMask) {
            ...
            unpaintedCtx.clearRect(0, 0, unpaintedCanvas.width, unpaintedCanvas.height);
            unpaintedCtx.globalCompositeOperation = "source-over";
            unpaintedCtx.drawImage(bodyImage, 0, 0, sprite.width, sprite.height);
            unpaintedCtx.globalCompositeOperation = "destination-out";
            unpaintedCtx.drawImage(paintMaskImage, 0, 0, sprite.width, sprite.height);
            unpaintedCtx.globalCompositeOperation = "source-over";
            ctx.drawImage(unpaintedCanvas, sprite.xOffset, sprite.yOffset, sprite.width, sprite.height);
        }
        ...
        if (car.paintColor) {
            ...
            paintCtx.drawImage(hasPaintMask ? paintMaskImage : bodyImage, 0, 0, sprite.width, sprite.height);
            paintCtx.globalCompositeOperation = "source-over";
        ...
        if (decal.imagePath) {
            ...
            const decalClipImage = hasPaintMask && paintMaskImage
                ? paintMaskImage
                : bodyImage;
            if (isImageReady(decalImage)) {
                ...
                decalCtx.drawImage(decalImage, 0, 0, sprite.width, sprite.height);
                if (decal.colorable) {
                    decalCtx.globalCompositeOperation = "source-in";
                    decalCtx.fillStyle = car.decalColor || decal.defaultColor || "#ffffff";
                    decalCtx.fillRect(0, 0, sprite.width, sprite.height);
                }
                decalCtx.globalCompositeOperation = "destination-in";
                decalCtx.drawImage(decalClipImage, 0, 0, sprite.width, sprite.height);
                decalCtx.globalCompositeOperation = "source-over";
                // ===== DECAL OVERLAY =====
                ctx.drawImage(decalCanvas, sprite.xOffset, sprite.yOffset, sprite.width, sprite.height);
            }
        }
    }
    ctx.restore();
    ...
    const wheelSpin = car.pos * 4.45 +
        car.spd * 2.95;
    const speedBlur = Math.min(speedToMph(Math.abs(car.spd)) / 160, 1);
...
function drawRoad(ctx, canvas, camX, playerX, direction, worldScale) {
    const roadTop = 145;
    const roadHeight = canvas.height - roadTop;
    const roadGradient = ctx.createLinearGradient(0, roadTop, 0, canvas.height);
    ...
    const dashHeight = 4;
    const dashY = 173;
    const offset = ((playerX - (camX * direction * worldScale)) % dashCycle + dashCycle) %
        dashCycle;
    ctx.fillStyle = "rgba(245, 245, 235, 0.78)";
    for (let x = offset - dashCycle; x < canvas.width + dashCycle; x += dashCycle) {
...
export const Render = {
    ...
    draw(player, ai) {
        ...
        for (let y = 0; y < canvas.height; y += tileSize) {
            ...
            ctx.fillStyle =
                row % 2 === 0
                    ? "#ffffff"
                    : "#000000";
            ctx.fillRect(finishDrawX, y, lineWidth, tileSize);
            ctx.fillStyle =
                row % 2 === 0
                    ? "#000000"
                    : "#ffffff";
            ctx.fillRect(finishDrawX + lineWidth, y, lineWidth, tileSize);
        }
        // ===== START LINE =====
        const startWorld = 0;
        const startX = playerX +
            ...
            (lineOffset * direction);
        const startDrawX = direction === -1
            ? startX - lineWidth
            : startX;
        ctx.fillStyle = "#008d38";
        ctx.fillRect(startDrawX, 0, lineWidth, canvas.height);
        // ===== LANE POSITIONS =====
        const topLaneY = 134;
        ...
        for (const carDraw of carsByLaneDepth) {
            drawCarBody(ctx, carDraw.car, carDraw.x, carDraw.y, direction);
        }
