...
function updateBoost(car, rpmRatio, dt) {
    if (!car)
        return 0;
    ...
    const throttle = getThrottleInput(car);
    ...
    let boostShape = 0.34 + Math.pow(rpmBoostFactor, 0.72) * 0.66;
    if (inductionType === "turbo") {
        const load = throttle *
            clamp((rpmRatio - 0.28) / 0.52, 0, 1);
        const targetSpool = load * rpmBoostFactor;
        const currentSpool = clamp(car.turboSpool || 0, 0, 1);
        const spoolRate = (0.9 + level * 0.18) *
            (car.shiftTimer > 0 ? 0.34 : 1);
        const unspoolRate = car.shiftTimer > 0
            ? 0.85
            : 1.75;
    ...
    const targetBoost = maxBoostPsi * boostShape * throttle;
    const response = inductionType === "turbo"
        ? 1.1 + level * 0.18
        : 8.5;
    const bleed = inductionType === "turbo"
        ? 5.8
        : 10.5;
    const currentBoost = Math.max(car.boostPsi || 0, 0);
...
export const Physics = {
    update(car, dt) {
        ...
        // ===== CURRENT GEAR DATA =====
        const gearIndex = car.gear - 1;
        const gearRatio = car.gearRatios[gearIndex] || car.gearRatios[0];
        const gearMaxSpeed = mphToSpeed(car.gearMaxSpeeds[gearIndex] || car.topSpeed);
        const maxCarSpeed = mphToSpeed(car.topSpeed);
        const weightRatio = Math.max(car.weight || 2800, 1) / 2800;
        const weightAccelFactor = Math.max(0.72, Math.min(Math.pow(1 / weightRatio, 0.52), 1.26));
        const weightGripFactor = Math.max(0.78, Math.min(Math.pow(1 / weightRatio, 0.42), 1.24));
        const spinWeightPenalty = Math.max(0.82, Math.min(Math.pow(weightRatio, 0.52), 1.34));
        // Prevent tach from freezing before redline
        // when vehicle top speed is lower than gear max speed.
        const effectiveGearMaxSpeed = Math.min(gearMaxSpeed, maxCarSpeed);
        ...
        const previousGearMaxSpeed = gearIndex === 0
            ? 0
            : Math.min(mphToSpeed(car.gearMaxSpeeds[gearIndex - 1] || 0), maxCarSpeed);
        const gearStartSpeed = gearIndex === 0
            ? 0
        ...
        const curveMultiplier = getCurveMultiplier(car.torqueCurve, rpmRatio);
        const boostPsi = updateBoost(car, rpmRatio, dt);
        const boostTorqueMultiplier = getBoostTorqueMultiplier(car, boostPsi, rpmRatio);
        const liveTorque = Math.max(1, (car.torque || 1) *
            curveMultiplier *
            boostTorqueMultiplier);
        car.liveTorque =
            liveTorque;
        car.liveHorsepower =
            Math.max(1, liveTorque * Math.max(car.rpm, 1000) / 5252);
        const highRpmPowerAccess = clamp((rpmRatio - 0.32) / 0.5, 0, 1);
        // ===== DRIVETRAIN FORCE =====
        const driveRatio = gearRatio * car.finalDrive;
        const accelRatio = 1 + ((driveRatio - 1) * 0.55);
        const torqueToWeight = liveTorque / car.weight;
        const powerToWeight = (((car.liveHorsepower || 1) * 0.72) +
            ((car.hp || 1) * highRpmPowerAccess * 0.28)) / car.weight;
        // ==== POWER TO TORQUE CONTROLLER ====
        let accel = ((powerToWeight * 0.52) + (torqueToWeight * 0.44)) *
            15 *
            SPEED_FEEL_SCALE;
        accel *=
            accelRatio *
        ...
        // ===== TRACTION =====
        const torqueLoad = liveTorque / car.weight;
        const gripRating = normalizeGripRating(car.grip);
        const effectiveGrip = (gripRating / 3) * weightGripFactor;
        ...
        const gearTractionRelief = Math.min(Math.max((car.gear - 1) * 0.24, 0), 0.78);
        const accelerationLoad = Math.min(Math.max(accel / (35 * SPEED_FEEL_SCALE), 0), 1);
        ...
        const launchGripTuning = (car.launchGrip || 4.5) / 4.5;
        const launchGrip = effectiveGrip *
            SPEED_FEEL_SCALE *
            (1.35 + launchPhase * 0.35 * launchGripTuning);
        const speedGrip = car.spd * (0.55 + rollingGripPhase * 0.75);
        const gripLimit = (launchGrip * drivetrainGripMultiplier) + speedGrip;
        const torqueSpinPressure = torqueLoad *
            65 *
            drivetrainSpinPressure *
            spinWeightPenalty *
            lowGripPenalty *
        ...
        const adjustedGripLimit = Math.max(gripLimit - torqueSpinPressure, gripLimit * 0.68);
        if (accel > adjustedGripLimit) {
            const excessAccel = accel - adjustedGripLimit;
            const tractionLoad = torqueLoad / Math.max(effectiveGrip, 0.1);
            const spinSeverity = Math.min(1.55, tractionLoad *
                34 *
                drivetrainSpinSeverity *
                spinWeightPenalty *
                lowGripPenalty *
            ...
            const usableSpinPower = Math.max(0.66, (1.75 - (spinSeverity * 1.22)) *
                drivetrainSpinRecovery /
                (spinWeightPenalty * lowGripPenalty));
            accel =
                adjustedGripLimit + (excessAccel * usableSpinPower);
            const spinSpeedPenalty = 0.0032 *
                spinWeightPenalty *
                lowGripPenalty *
